this.sleep_player <- this.inherit("scripts/skills/skill", {
	...
	function getHitchance( _targetEntity )
	{
		...
		local chance_defend = this.Math.floor((target.getBravery()/(chance_attack) - 0.20 + modifier_defender_morale) * 100);
		...
		ret.extend([
			{
				...
				text = "[color=" + this.Const.UI.Color.PositiveValue + "] -20%[/color] (Base hit chance)"
			}
		]);
